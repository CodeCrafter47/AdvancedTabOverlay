...
class OrderedTabOverlayHandler implements TabOverlayHandler {
    ...
    private abstract class CustomOperationModeHandler<R extends CustomOperationModeHandler<R>.Representation> extends OperationModeHandler<R> {
            ...
            if (type == PacketType.Play.Server.PLAYER_INFO) {
                ...
                if (!infoDataList.isEmpty() && !fakePlayerUUIDSet.contains(infoDataList.get(0).getProfile().getUUID())) {
                    ...
                    if (action.contains(EnumWrappers.PlayerInfoAction.UPDATE_LISTED)) {
                        packet.getPlayerInfoDataLists().write(1, infoDataList.stream().map(playerInfoData -> {
                            return new PlayerInfoData(playerInfoData.getProfileId(), playerInfoData.getLatency(), false, playerInfoData.getGameMode(), playerInfoData.getProfile(), playerInfoData.getDisplayName(), playerInfoData.isShowHat(), playerInfoData.getRemoteChatSessionData());
                        }).collect(Collectors.toList()));
                    }
                }
            }
        ...
        class Representation implements TabOverlayHandle.BatchModifiable, TabOverlayHandle {
            ...
            void updateSlot(int index) {
                ...
                val profile = new WrappedGameProfile(fakePlayerUUIDs[index], "");
                ...
                val data = new PlayerInfoData(profile.getUUID(), ping, true, EnumWrappers.NativeGameMode.SURVIVAL, profile, WrappedChatComponent.fromJson(text), -index, null);
                val packet = ProtocolLibrary.getProtocolManager().createPacket(PacketType.Play.Server.PLAYER_INFO);
                packet.getPlayerInfoActions().write(0, ImmutableSet.of(EnumWrappers.PlayerInfoAction.ADD_PLAYER, EnumWrappers.PlayerInfoAction.UPDATE_DISPLAY_NAME, EnumWrappers.PlayerInfoAction.UPDATE_LATENCY, EnumWrappers.PlayerInfoAction.UPDATE_LISTED, EnumWrappers.PlayerInfoAction.UPDATE_LIST_ORDER));
    ...
    private final class FixedSizeOperationModeHandler extends CustomOperationModeHandler<FixedSizeOperationModeHandler.Representation> {
        ...
        class Representation extends CustomOperationModeHandler<Representation>.Representation implements RectangularTabOverlay {
    ...
    private final class SimpleOperationModeHandler extends CustomOperationModeHandler<SimpleOperationModeHandler.Representation> {
        ...
        class Representation extends CustomOperationModeHandler<Representation>.Representation implements SimpleTabOverlay {
